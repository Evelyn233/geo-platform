...
"""
简单的LightRAG问答系统
直接使用LightRAG进行问答，避免RAG-Anything的复杂性
"""
...
async def test_lightrag_qa():
    ...
    for i, question in enumerate(questions, 1):
        ...
        try:
            ...
            result = await lightrag.aquery(question, mode="naive")
            print(f"回答: {result}")
        except Exception as e:
            print(f"查询失败: {e}")
...
async def interactive_lightrag_qa():
    """交互式LightRAG问答"""
    print("=== 交互式LightRAG问答 ===")
    ...
    while True:
        try:
            ...
            print("正在思考...")
            answer = await lightrag.aquery(user_input, mode=current_mode)
            print(f"回答: {answer}")
...
def main():
    ...
    if choice == "1":
        asyncio.run(test_lightrag_qa())
    elif choice == "2":
        asyncio.run(interactive_lightrag_qa())
    elif choice == "3":
        print("再见！")
    else:
        print("无效选择")
